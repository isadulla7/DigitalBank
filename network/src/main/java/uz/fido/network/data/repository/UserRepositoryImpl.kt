...
import uz.fido.network.domain.datasource.repositories.IUserRepository
...
class UserRepositoryImpl @Inject constructor(private val userApiService: UserApiInterface) : IUserRepository {
    ...
    override suspend fun getPaymentFile(
        ...
        userApiService.getPaymentFile(token)
    }

    override suspend fun checkUserSignIn(
        requestBody: CheckUserSignInRequest
    ): Resource<BaseResponse> = getResult {
        userApiService.checkUserSignIn(requestBody)
    }
    ...
    override suspend fun signUpCheck(
        ...
        userApiService.signUpCheck(signUpCheckRequest)
    }
