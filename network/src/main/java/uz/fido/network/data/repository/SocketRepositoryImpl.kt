...
import uz.fido.network.domain.datasource.repositories.ISocketRepository
...
class SocketRepositoryImpl @Inject constructor(private val socketService: SocketInterface) :
    ISocketRepository {
    override suspend fun testSocket(token: String): Resource<Any> = getResult {
        socketService.testSocket(token)
    }
}