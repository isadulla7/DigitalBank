...
class ConfirmTransferFragment :
    BaseFragment<FragmentConfirmTransferBinding, ConfirmTransferViewModel>(
        ...
        binding.tvReceivedAmount.text =
            "${
                Format.conversionFormat(
                    transferDto.transferAmount?.toDouble()?.div(100) ?: 0.0
                )
            } ${getString(uz.fido.utils.R.string.sum)}"
        setCommission(
            transferDto.commission.toString(),
            transferDto.transferAmount?.toDouble()?.div(100) ?: 0.0
        )
        setTotalAmount(
            transferDto.commission.toString(),
    ...
    private fun setTotalAmount(commission: String, amount: Double) {
        ...
        binding.tvTotalAmount.text =
            "${Format.conversionFormat(totalAmount)} ${getString(uz.fido.utils.R.string.sum)}"
    }

    private fun setCommission(commission: String, amount: Double) {
        val calculatedCommission = amount * commission.toDouble() / 100
        binding.tvCommission.text =
            commission + " % ($calculatedCommission ${getString(uz.fido.utils.R.string.sum)})"
    }
    ...
    private fun p2pRequest() {
        ...
        viewModel.p2pRequest(
            ...
            when (it.status) {
                Status.SUCCESS -> {
                    gotoWithSlide(
                        R.id.successTransferFragment, bundleOf(
                            SuccessTransferFragment.TRANSFER_DTO to transferDto
                        )
                    )
                }
