...
import uz.fido.universaldigital.databinding.ItemAddTemplateBinding
import uz.fido.universaldigital.databinding.ItemPaymentTemplateBinding
import uz.fido.universaldigital.ui.utils.extensions.loadTemplateImage

class PaymentTemplatesAdapter(
    private val baseInterface: BaseInterface
) : ListAdapter<Template, RecyclerView.ViewHolder>(TemplatesDiffCallback()) {

    inner class ViewHolderAdd(private val binding: ItemAddTemplateBinding) :
    ...
    inner class ViewHolder(private val binding: ItemPaymentTemplateBinding) :
        RecyclerView.ViewHolder(binding.root) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == 0) {
            ViewHolderAdd(
                ItemAddTemplateBinding.inflate(
                    LayoutInflater.from(parent.context), parent, false
                )
            )
        } else {
            ViewHolder(
                ItemPaymentTemplateBinding.inflate(
                    LayoutInflater.from(parent.context), parent, false
                )
            )
        }
    ...
    private class TemplatesDiffCallback : DiffUtil.ItemCallback<Template>() {
        override fun areItemsTheSame(oldItem: Template, newItem: Template): Boolean =
            oldItem.template_id == newItem.template_id
