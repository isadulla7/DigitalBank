...
class HumoMonitoringAdapter(
    ...
    inner class GeneralItemViewHolder(private val binding: ItemMonitoringBinding) :
        ...
        fun bind(item: ListItem) {
            ...
            binding.apply {
                monitoringItem?.let { humoMonitoringItem ->
                    tvName.setTransactionName(itemView.context, humoMonitoringItem)
                    tvTime.setTransactionTime(humoMonitoringItem)
                    tvType.setFormattedCardNumber(itemView.context, humoMonitoringItem)
                    tvAmount.setTransactionAmount(itemView.context, humoMonitoringItem)
                    tvAmount.setTextColor(itemView.context, humoMonitoringItem)
                    icon.setMonitoringImage(humoMonitoringItem)
                    father.setOnClickListener {
                        onClickView.invoke(humoMonitoringItem)
                    }
                }
            }
        }
