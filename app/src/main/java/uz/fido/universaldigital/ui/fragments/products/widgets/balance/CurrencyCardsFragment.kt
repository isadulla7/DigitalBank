...
class CurrencyCardsFragment : BaseSimpleFragment<FragmentBalaceByCardBinding>(
    ...
    override fun onInit(savedInstanceState: Bundle?) {
        ...
        currencyCards = MainBalanceDialog.currencyCards
        initTickerView()
        initCardList(currencyCards)
        setTotalBalance()
        binding.emptyText.isVisible = currencyCards.isEmpty()
        ...
        binding.switchMainBalance.setOnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                Paper.book().write(Const.TOTAL_BALANCE_TYPE, 1)
    ...
    override fun onResume() {
        super.onResume()
        binding.switchMainBalance.isChecked = Paper.book().read(Const.TOTAL_BALANCE_TYPE, 0) == 1
    }
    ...
    private fun setTotalBalance() {
        ...
        binding.tvTotalBalance.text =
            Format.formatAmount(Format.convertFromTiynDivide(totalBalance.toString())) + " " + "USD"
    }
}