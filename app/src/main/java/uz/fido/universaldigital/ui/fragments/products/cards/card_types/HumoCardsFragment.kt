...
@AndroidEntryPoint
class HumoCardsFragment : BaseSimpleFragment<FragmentOtherCardsBinding>(
    ...
    private var humoCardList = ArrayList<CardResponse>()
    ...
    private fun initCards() {
        ...
        cardsAdapter?.submitList(humoCardList)
        if (humoCardList.isEmpty()) showEmptyView(humoCardList)
    }

    private fun initCardsRv(cardLayoutManager: GridLayoutManager) {
        cardsAdapter = CardsListAdapter(this@HumoCardsFragment, cardLayoutManager)
    ...
    private fun refreshCards() {
        try {
            cardsAdapter = CardsListAdapter(this@HumoCardsFragment, getLayoutManager())
    ...
    override fun selectedCard(card: CardResponse) {
        cardOperationsDialog = CardOperationsDialog(card, this@HumoCardsFragment)
        ...
        when (p0?.id) {
            R.id.transfer_to_card -> {
                ...
                if (selectedCard.object_type != CardConst.CURRENCY_CARD) goto(
                    ...
                    bundleOf(Const.RECEIVER_CARD to selectedCard)
                ) else goto(
                    R.id.conversionFragment,
                    bundleOf(Const.SENDER_CARD to selectedCard)
                )
            }
            ...
            R.id.card_settings -> {
                ...
                goto(
                    ...
                    bundleOf(Const.CARD to selectedCard)
                )
            }
            ...
            it?.let {
                ...
                when (it.status) {
                    Status.SUCCESS -> {
                        ...
                        cardsAdapter?.notifyItemChanged(humoCardList.indexOf(selectedCard))
                    }
