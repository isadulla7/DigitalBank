...
class MyHomeViewModel @Inject constructor(
    ...
    fun createTemplateGroup(token: String, createTemplateGroupRequest: CreateTemplateGroupRequest) =
        liveData(Dispatchers.IO) {
            emit(templateRepository.createTemplateGroup(token, createTemplateGroupRequest))
        }

    fun deleteTemplateGroup(token: String, getTemplateListRequest: GetTemplateListRequest) =
        liveData(Dispatchers.IO) {
            emit(templateRepository.deleteTemplateGroup(token, getTemplateListRequest))
        }

    fun editTemplateGroup(token: String, editTemplateGroupRequest: EditTemplateGroupRequest) =
        liveData(Dispatchers.IO) {
