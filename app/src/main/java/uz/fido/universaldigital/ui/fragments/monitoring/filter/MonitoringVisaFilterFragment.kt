...
import kotlinx.android.synthetic.main.choose_card_dialog.father_bottom_sheet
import kotlinx.android.synthetic.main.fragment_monitoring_filter.card
...
class MonitoringVisaFilterFragment:BaseFragment<FragmentMonitoringUzcardFilterBinding,MonitoringFilterViewModel>
    ...
    private val monitoringFilterAdapter by lazy { MonitoringFilterAdapter(requireContext(), arrayListOf(),this) }
    ...
    private fun localFilterDone() {
        if (filterSaveVh!!.startDate!=""){
            ...
            dateCurrent=true
            binding.time.background=resources.getDrawable(R.drawable.monitoring_filter_item_color_click)
        ...
        if (filterSaveVh!!.maxAmount!=""){
            ...
            amountCurrent=true
            binding.amount.background=resources.getDrawable(R.drawable.monitoring_filter_item_color_click)
        ...
        if (filterSaveVh!!.plusMinus!=""){
            ...
            chooseCurrent=true
            binding.minPlus.background=resources.getDrawable(R.drawable.monitoring_filter_item_color_click)
    ...
    private fun getCardList() {
        ...
        viewModel.getLocalMonitoringCardList(getClientToken()).observe(viewLifecycleOwner){ resource->
            Handler().postDelayed({
                skeletonScreen.hide()
                binding.shimmerView.visibility=View.GONE},500)
            when(resource.status){
                Status.SUCCESS->{
                    val response=resource.data?.user_objects?: ArrayList()
                    val newList= arrayListOf<FilterCard>()
                    response.forEach{if (it.object_type=="TET")newList.add(it)}
    ...
    private fun showAmountFilter() {
        if (!amountCurrent){
            ...
            maxAmount=""
            amountCurrent=false
            binding.amount.background=resources.getDrawable(R.drawable.monitoring_filter_item_color)
    ...
    private fun showChoose() {
        if (!chooseCurrent){
            ...
            this.choose=""
            chooseCurrent=false
            binding.minPlus.background=resources.getDrawable(R.drawable.monitoring_filter_item_color)
